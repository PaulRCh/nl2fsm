...
def generate_automaton_prompt(nbState):
    mm = gm.generaterandommm(nbState) # Generate a random automaton
    nl = gd.generatedescription(mm) # Generate a natural language description of the automaton
    return mm, mm.toDot(), nl # Return the automaton, the dot representation of the automaton and the natural language description

def generate_automaton_prompt_with_random_nb_states():
    nbState = random.randrange(6,15) # Generate a random number of states
    return generate_automaton_prompt(nbState) # Return the automaton prompt

def nl_to_prompt(nl): # Convert the natural language description to a prompt
...
def nl_to_prompt2(nl): # Convert the natural language description to a prompt (used only for testing)
...
def get_user_input(): # Get the user description of an automaton or generate a random one

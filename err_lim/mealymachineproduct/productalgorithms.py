...
class ProductMealyMachine(MealyMachine) :
    ...
    def caracterize_errors(self):
        m_n_states = len(self.machine1.get_states())
        m_alphabet_size = len(self.machine1.get_input_alphabet())
        n_type1_errors = 0
        n_type2_errors = 0
        n_type3_errors = 0
        n_type4_errors = 0
        for i in range(m_n_states):
            state_m1 = self.machine1.get_state(i)
            state_m2 = self.machine2.get_state(i)
            for j in range(m_alphabet_size):
                input = self.machine1.get_input_alphabet()[j]
                trs1 = state_m1.get_out_transition(input)
                try:
                    trs2 = state_m2.get_out_transition(input)
                except KeyError:
                    trs2 = []
                if len(trs1) < len(trs2):
                    print(f"Error, there is more transitions in state {state_m1.get_name()} in machine 2 than in machine 1 for input {input}")
                    n_type1_errors += 1
                if len(trs1) == 1 and len(trs2) == 0:
                    print(f"Error, there is no transition in state {state_m1.get_name()} in machine 2 for input {input}")
                    n_type2_errors += 1
                else:
                    tr1 = trs1[0]
                    tr2 = trs2[0]
                    output1 = tr1.get_output()
                    output2 = tr2.get_output()
                    tgt1 = tr1.get_tgt()
                    tgt2 = tr2.get_tgt()
                    if output1 != output2:
                        print(f"Error, the output of the transition in state {state_m1.get_name()} in machine 1: {output1}, is different from the output in machine 2: {output2}, for input {input}")
                        n_type3_errors += 1
                    if tgt1.get_id() != tgt2.get_id():
                        print(f"Error, the target of the transition in state {state_m1.get_name()} in machine 1: {tgt1.get_name()}, is different from the target in machine 2: {tgt2.get_name()}, for input {input}")
                        n_type4_errors += 1
        return n_type1_errors, n_type2_errors, n_type3_errors, n_type4_errors
